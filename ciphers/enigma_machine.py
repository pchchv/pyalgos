"""
This module contains function 'enigma'
which emulates the famous Enigma machine from WWII.
...
{'P': 'O', 'O': 'P', 'L': 'A', 'A': 'L', 'N': 'D', 'D': 'N'})
    ...
    if not 0 < rotorpos1 <= len(abc):
        raise ValueError(
            "First rotor position is not \
                within range of 1..26 (" f"{rotorpos1}"
        )
    if not 0 < rotorpos2 <= len(abc):
        raise ValueError(
            "Second rotor position is not \
                within range of 1..26 (" f"{rotorpos2})"
        )
    if not 0 < rotorpos3 <= len(abc):
        raise ValueError(
            "Third rotor position is not \
                within range of 1..26 (" f"{rotorpos3})"
        )
...
def _plugboard(pbstring: str) -> dict[str, str]:
    ...
    if not isinstance(pbstring, str):
        raise TypeError(f"Plugboard setting isn't \
                        type string ({type(pbstring)})")
...
if __name__ == "__main__":
    ...
    message = "This is Python script that emulates \
        the Enigma machine from WWII."
    rotor_pos = (1, 1, 1)
    pb = "pictures"
