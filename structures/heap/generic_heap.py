...
class Heap:
    ...
    def __init__(self, key: Callable | None = None) -> None:
        ...
        self.size = 0
        # Stores the function used to estimate the score of
        # the item on the basis of which the ordering will be made.
    ...
    def _get_valid_parent(self, i: int) -> int:
        """Returns the index of the actual parent according to
        the desired order among the given index and its children.
        """
        left = self._left(i)
        right = self._right(i)
        valid_parent = i
    ...
    def _heapify_down(self, index: int) -> None:
        ...
        while valid_parent != index:
            ...
            index, valid_parent = \
                valid_parent, self._get_valid_parent(valid_parent)
    ...
    def delete_item(self, item: int) -> None:
        ...
        self.size -= 1
        # Make sure that the heap is positioned correctly in
        # both the up and down directions.
        # Ideally, only one of them will make changes,
        # so calling both will not result in a loss of performance.
        if self.size > index:
            self._heapify_up(index)
            self._heapify_down(index)
    ...
    def extract_top(self) -> tuple | None:
        """Return top item tuple
        (Calculated value, item) from heap and removes it as well if present.
        """
