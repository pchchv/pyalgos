...
class BinomialHeap:
    r"""
    Min-oriented priority queue implemented with Binomial Heap data structure
    implemented with BinomialHeap class. It supports:
    - Inserting an element into a heap with n elements:
       Guaranteed logn, amortized 1.
    - Merge (meld) heaps of size m and n: O(logn + logm)
    - Delete min: O(logn)
    - Peek (return min without removing it): O(1)

    Example:
    Create a random permutation of 30 integers to be
    inserted and 19 of them to be deleted.
    >>> import numpy as np
    ...
    def merge_heaps(self, other):
        ...
        for i in range(len(combined_roots_list) - 1):
            if combined_roots_list[i][1] != combined_roots_list[i + 1][1]:
                combined_roots_list[i][0].parent = \
                    combined_roots_list[i + 1][0]
    ...
    def insert(self, val):
        ...
        if self.size == 0:
            ...
            while (
                ...
                # Merge
                self.bottom_root = \
                    self.bottom_root.merge_trees(self.bottom_root.parent)
    ...
    def __str__(self):
        ...
        return "\n".join(("-" * level + str(value)) for value,
                         level in preorder_heap)
