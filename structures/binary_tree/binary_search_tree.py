...
class BinarySearchTree:
    ...
    def __insert(self, value) -> None:
        ...
        if self.empty():  # if Tree is empty
            ...
            if parent_node is None:
                return
            while True:  # Before getting to the leaf
                if value < parent_node.value:  # Go left
                    if parent_node.left is None:
                        # Insert the new node in a leaf
    ...
    def get_max(self, node: Node | None = None) -> Node | None:
        """
        Going inland on the right branch.
        """
        if node is None:
    ...
    def remove(self, value: int) -> None:
        ...
        if node is not None:
            ...
            if node.left is None and node.right is None:
                ...
                )  # Assigns value to node to remove and save tree structure
    ...
    def find_kth_smallest(self, k: int, node: Node) -> int:
        ...
        arr: list[int] = []
        self.inorder(arr, node)  # all values to list using inorder traversal
        return arr[k - 1]
