"""
Implementation of the Skew Heap.
...
class SkewHeap(Generic[T]):
    """
    A data structure that allows you to insert a
    new value and output the smallest value.
    Both operations take time O(logN),
    ...
    def pop(self) -> T | None:
        ...
        result = self.top()
        self._root = (
            SkewNode.merge(self._root.left,
                           self._root.right) if self._root else None
        )
