...
def encrypt_and_write_to_file(
    ...
    if key_size < block_size * 8:
        sys.exit(
            "ERROR: Block size is %s bits and key size is %s bits. \
                The RSA cipher "
            "requires the block size to be equal to or \
                greater than the key size. "
            "Either decrease the block size or use \
                different keys."
            % (block_size * 8, key_size)
        )

    encrypted_blocks = [str(i) for i in encrypt_message(message,
                                                        (n, e),
                                                        block_size)]
...
def read_from_file_and_decrypt(message_filename: str,
                               key_filename: str) -> str:
    ...
    if key_size < block_size * 8:
        sys.exit(
            "ERROR: Block size is %s bits and key size is %s bits. \
                The RSA cipher "
            "requires the block size to be \
                equal to or greater than the key size. "
    ...
    return decrypt_message(encrypted_blocks,
                           message_length,
                           (n, d),
                           block_size)
...
def main() -> None:
    ...
    if mode == "encrypt":
        ...
        encrypted_text = encrypt_and_write_to_file(filename,
                                                   pubkey_filename,
                                                   message)
