"""
The MD5 algorithm is a hash function
that is typically used as a checksum to detect data corruption.
The algorithm works by processing a given message in blocks of 512 bits,
augmenting the message as needed. It uses blocks to operate in
a 128-bit state and performs a total of 64 such operations.
Note that all values are little-endian, so input data is converted as needed.
Although MD5 has been used as a cryptographic hash function in the past,
it has since been cracked, so it should not be used for security purposes.
...
def preprocess(message: bytes) -> bytes:
    ...
    while len(bit_string) % 512 != 448:
        bit_string += b"0"
    bit_string += to_little_endian(start_len[32:]) + \
        to_little_endian(start_len[:32])

    return bit_string


def get_block_words(bit_string: bytes) -> Generator[list[int], None, None]:
    """Splits bit string into blocks of 512 chars and
    yields each block as a list of 32-bit words.
    Example: Suppose the input is the following:
        bit_string =
...
def sum_32(a: int, b: int) -> int:
    """Adds two numbers as 32-bit ints.
    Arguments:
...
def md5_me(message: bytes) -> bytes:
    ...
    for block_words in get_block_words(bit_string):
        ...
        for i in range(64):
            if i <= 15:
                # f = (b & c) | (not_32(b) & d)     # Alternate definition for f  # noqa: E501
                f = d ^ (b & (c ^ d))
                g = i
            elif i <= 31:
                # f = (d & b) | (not_32(d) & c)     # Alternate definition for f  # noqa: E501
                f = c ^ (d & (b ^ c))
                g = (5 * i + 1) % 16
            elif i <= 47:
                f = b ^ c ^ d
    ...
    digest = reformat_hex(a0) + reformat_hex(b0) + \
        reformat_hex(c0) + reformat_hex(d0)
    return digest
