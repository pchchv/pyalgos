...
class SkipList(Generic[KT, VT]):
    ...
    def __str__(self) -> str:
        ...
        lines.append(f"[{node.key}]".ljust(label_size, "-") + "* \
                     " * len(forwards))
        ...
        while len(node.forward) != 0:
            ...
            lines.append(
                ...
                + " ".join(str(n.key) if n.key == node.key else "|\
                           " for n in forwards)
            )
            lines.append(" " * label_size + "| " * len(forwards))
    ...
    def _locate_node(self, key) -> tuple[Node[KT, VT] | None,
                                         list[Node[KT, VT]]]:
        """
        :param key: Searched key,
        :return: The tuple with the searched node
            (or None if the given key is absent) and
            the list of nodes that refer (if the key is present) to
            the node must refer to this node.
        """
        ...
        for i in reversed(range(self.level)):
            ...
            while i < node.level and node.forward[i].key < key:
                node = node.forward[i]
            # Each leftmost node (relative to searched node)
            # will potentially have to be updated.
            update_vector.append(node)

        update_vector.reverse()  # Values are inserted in reverse order.

        # len(node.forward) != 0 - If current node doesn't contain any further
        #                          references then searched key is not present.
        # node.forward[0].key == key - Next node key should be equal to
        #                              search key if key is present.
    ...
    def find(self, key: VT) -> VT | None:
        ...
        :return: Value associated with given key or
        None if given key is not present.
        >>> skip_list = SkipList()
        >>> skip_list.find(2)
        >>> skip_list.insert(2, "Two")
        >>> skip_list.find(2)
